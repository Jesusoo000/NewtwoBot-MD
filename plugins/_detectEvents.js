...
await this.sendMessage(m.chat, { text: `『✦』El participante: ${usuario} ha modificado el nombre del grupo.`, mentions: [m.sender], mentions: (await conn.groupMetadata(m.chat)).participants.map(v => v.id) }, { quoted: m,, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100}) 
...
await this.sendMessage(m.chat, { text: `『✦』Se ha configurado los ajustes del grupo, ${m.messageStubParameters[0] == 'on' ? 'solo los admins' : 'ahora todos'} pueden configurar los ajustes del grupo.`, mentions: [m.sender] }, { quoted: m,, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})
...
await this.sendMessage(m.chat, { text: `『✦』El participante: ${usuario} ha configurado los ajustes temporales a: @${m.messageStubParameters[0]}*`, mentions: [m.sender] }, { quoted: m,, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})

} else if (chat.detect2 && m.messageStubType == 123) {
await this.sendMessage(m.chat, { text: `『✦』El participante: ${usuario} ha desactivado los mensajes temporales.`, mentions: [m.sender] }, { quoted: m, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})
} else {
